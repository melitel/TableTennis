...
void Wall::initialize()
{
	...
	if (m_wall_type == type::top || m_wall_type == type::bottom) {
	...
	else if (m_wall_type == type::left || m_wall_type == type::right) {
		m_wall_shape.setSize(sf::Vector2f(m_wall_height, (m_wall_width - 100)));
		statactor->initialize(m_position, Vector2f(m_wall_height, (m_wall_width - 100)), get_hit_normal());
	}
	else {
		assert(false);
	}	
...
Vector2f Wall::get_hit_normal()
{
	...
	else if (m_wall_type == type::left) {
		hit_normal = Vector2f(1.f, 0.f);
	}
	else if (m_wall_type == type::right) {
		hit_normal = Vector2f(-1.f, 0.f);
	}
	else {
		assert(false);
