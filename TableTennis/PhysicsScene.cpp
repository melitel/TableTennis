...
void PhysicsScene::initialize(int width, int height)
{
	...
	for (int i = 0; i < m_sector.size(); i++) {
		for (auto dynactor : m_dynamic_actors) {
			if (m_sector[i].get_bounds().intersects(dynactor->get_bounds())) {
				dynactor->set_sector(i);
				m_sector[i].add_dynamic_actor(dynactor);
			}
		}
		for (auto statactor : m_static_actors) {
			if (m_sector[i].get_bounds().intersects(statactor->get_bounds())) {
				statactor->set_sector(i);
				m_sector[i].add_dynamic_actor(statactor);
			}
		}
	}
}
...
void PhysicsScene::simulate(float delta, float round_time)
{
	...
	for (auto& actor : m_dynamic_actors) {
		...
		if (dynactor->isPureDynamic()) {
			sf::Vector2f p0 = dynactor->get_position();
			sf::Vector2f p1 = p0 + delta * dynactor->get_velocity();
...
{
	m_dynamic_actors.push_back(std::make_shared<PhysicActor>(g_Game->physic_scene(), owner));
...
{
	m_static_actors.push_back(std::make_shared<PhysicActor>(g_Game->physic_scene(), owner));
